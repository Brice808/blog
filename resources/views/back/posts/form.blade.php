        ...
        <div class="row">
            <div class="col-md-8">
                ...
                <x-back.card type='primary' title='Title'>
                    <x-back.input name='title' :value="isset($post) ? $post->title : ''" input='text' :required="true">
                    </x-back.input>
                </x-back.card>

                <x-back.card type='primary' title='Excerpt'>
                    <x-back.input name='excerpt' :value="isset($post) ? $post->excerpt : ''" input='textarea'
                        :required="true">
                    </x-back.input>
                </x-back.card>

                <x-back.card type='primary' title='Body'>
                    <x-back.input name='body' :value="isset($post) ? $post->body : ''" input='textarea' rows=10
                        :required="true">
                    </x-back.input>
                </x-back.card>
            ...
            <div class="col-md-4">
                ...
                <x-back.card type='warning' :outline="false" title='Categories' :required="true">
                    <x-back.input name='categories' :values="isset($post) ? $post->categories : collect()"
                        input='selectMultiple' :options="$categories">
                    </x-back.input>
                </x-back.card>

                <x-back.card type='danger' :outline="false" title='Tags'>
                    <x-back.input name='tags'
                        :value="isset($post) ? implode(',', $post->tags->pluck('tag')->toArray()) : ''" input='text'>
                    </x-back.input>
                </x-back.card>
                ...
                <x-back.card type='primary' :outline="false" title='Image'>
                    ...
                    <div class="input-group mb-3">
                        ...
                        <input id="image" class="form-control {{ $errors->has('image') ? 'is-invalid' : '' }}" type="text"
                            name="image" value="{{ old('image', isset($post) ? getImage($post) : '') }}" required>
                        @if ($errors->has('image'))
